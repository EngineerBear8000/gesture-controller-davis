      ...
      if (SSID.indexOf("Slave") == 0) {
        ...
        if ( 6 == sscanf(BSSIDstr.c_str(), "%x:%x:%x:%x:%x:%x",  &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5] ) ) {
          for (int ii = 0; ii < 6; ++ii ) {
            ...
            // Serial.println(slaves[SlaveCnt].peer_addr[ii]);
          }
        }
...
void setup() {
  ...
  pinMode(dropRPin,INPUT_PULLUP);
  pinMode(dropGPin,INPUT_PULLUP);
...
void loop() {
  ...
  if (SlaveCnt > 0) { // check if slave channel is defined
    ...
    // Send data to device
    dropR = digitalRead(dropRPin);
    dropG = digitalRead(dropGPin);
    Serial.print("drop Red:");
    Serial.print(dropR);
    Serial.print("drop Green:");
    Serial.println(dropG);
        
    if(dropR == 0 && dropG == 1){
      data = 1;  //drop Red button pressed so send 1
    }
    else if(dropR == 1 && dropG == 0){
      data = 2;  //drop green button pressed so send 2
    }
    else{
      data = 0; //if either both pressed or none pressed send 0
    }
    sendData();
  } 
